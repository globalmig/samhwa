...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const before = await prisma.taxInvoice.findUnique({ where: { id }, include: INCLUDE });
  if (!before) return Response.json({ ok: false, error: "세금계산서를 찾을 수 없습니다." }, { status: 404 });

  const updated = await prisma.$transaction(async (tx) => {
    const row = await tx.taxInvoice.update({
      where: { id },
      data: {
    ...
    const afterInvoice = toTaxInvoice(row);
    await writeAuditLog(tx, {
      actorUserId: actor.userId,
      entityType: "taxInvoice",
      entityId: row.id,
      entityLabel: afterInvoice.invoiceNumber,
      action: "UPDATE",
      before: toTaxInvoice(before) as unknown as Record<string, unknown>,
