    ...
    <div className="space-y-5">
      ...
      <section>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <Link
            href="/receivables"
            className="block bg-white rounded-xl border border-slate-200 px-4 py-3 hover:border-red-300 hover:shadow-sm transition-all group"
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {overdueReceivables.length > 0 && (
                <span className="text-[10px] font-bold text-white bg-red-500 rounded-full px-1.5 py-0.5 leading-none">
                  {overdueReceivables.length}건
                </span>
              )}
            </div>
            <p className={`text-base font-bold ${overdueReceivables.length > 0 ? "text-red-600" : "text-slate-300"}`}>
              {overdueReceivables.length > 0 ? fmt(overdueAmount) + "원" : "없음"}
            </p>
            <p className="text-[10px] text-slate-400 mt-1.5 group-hover:text-red-500 transition-colors">
              채권 페이지에서 관리 →
            </p>
          </Link>
          ...
          <Link
            href="/issues"
            className="block bg-white rounded-xl border border-slate-200 px-4 py-3 hover:border-amber-300 hover:shadow-sm transition-all group"
          >
            <div className="flex items-center justify-between mb-2">
            ...
            <p className={`text-base font-bold ${highIssues.length > 0 ? "text-amber-600" : "text-slate-300"}`}>
              {highIssues.length > 0 ? `미처리 ${highIssues.length}건` : "없음"}
            </p>
            <p className="text-[10px] text-slate-400 mt-1.5 group-hover:text-amber-500 transition-colors">
              이슈 페이지에서 확인 →
            </p>
          </Link>
          ...
          <Link
            href="/settlements"
            className="block bg-white rounded-xl border border-slate-200 px-4 py-3 hover:border-blue-300 hover:shadow-sm transition-all group"
          >
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-[10px] text-slate-400 mt-1.5 group-hover:text-blue-500 transition-colors">
              정산 페이지에서 관리 →
            </p>
          </Link>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              {pendingUnclaimed.length > 0 && (
                <span className="text-[10px] font-bold text-white bg-amber-500 rounded-full px-1.5 py-0.5 leading-none">
                  {pendingUnclaimed.length}건
                </span>
              )}
            </div>
            <p className={`text-base font-bold ${pendingUnclaimed.length > 0 ? "text-amber-600" : "text-slate-300"}`}>
              {pendingUnclaimed.length > 0 ? fmt(pendingUnclaimedAmount) + "원" : "없음"}
            </p>
            <p className="text-[10px] text-slate-400 mt-1.5 group-hover:text-amber-500 transition-colors">
              미청구 페이지에서 관리 →
            </p>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <Link
              key={card.label}
              href={card.href}
              className="block bg-white rounded-xl border border-slate-200 px-4 py-3 hover:border-blue-300 hover:shadow-sm transition-all"
            >
              ...
              {card.change && (
                <p className="text-[10px] text-emerald-600 mt-1 font-medium">{card.change} 전월비</p>
              )}
            </Link>
          ))}
        </div>
      ...
      <section className="bg-white rounded-xl border border-slate-200 px-5 py-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <Link
              key={item.label}
              href={`/projects?status=${item.status}`}
              className={`block ${item.bg} rounded-lg px-4 py-3 ring-1 ring-transparent ${item.hover} transition-all`}
            >
          ...
          <table className="w-full text-sm">
              ...
              {agencyFeeRows.map((row, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-right text-sm text-slate-700 tabular-nums">
                      {row.issuedAmount.toLocaleString("ko-KR")}
                    </td>
                    <td className="px-4 py-3.5 text-right text-sm text-slate-700 tabular-nums">
                      {row.collectedAmount.toLocaleString("ko-KR")}
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right tabular-nums">
                      <span
                        className={`text-sm font-semibold ${
                          rate >= 90 ? "text-emerald-600" : rate >= 50 ? "text-blue-600" : "text-red-500"
                        }`}
                      >
                        {row.issuedAmount > 0 ? fmtRate(rate) : "–"}
                      </span>
                    </td>
            ...
            <tfoot>
              <tr className="bg-slate-50 border-t border-slate-200">
                ...
                <td className="px-4 py-3 text-right text-xs font-semibold text-slate-700 tabular-nums">
                  {totalIssued.toLocaleString("ko-KR")}
                </td>
                <td className="px-4 py-3 text-right text-xs font-semibold text-slate-700 tabular-nums">
                  {totalCollected.toLocaleString("ko-KR")}
                </td>
                <td className="px-4 py-3 text-center text-xs font-semibold text-slate-700">
                  {totalIssuedCount}건
                </td>
                <td className="px-4 py-3 text-right text-xs font-semibold text-blue-600 tabular-nums">
                  {totalIssued > 0 ? fmtRate((totalCollected / totalIssued) * 100) : "–"}
                </td>
                <td />
              </tr>
            </tfoot>
