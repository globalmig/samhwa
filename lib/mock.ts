...
export interface Institution {
  ...
  // 기관의 회계담당자 — 세금계산서·공문 발송(과제 상세 페이지 "공문 발송" 버튼)의 수신처로 쓰인다.
  // 삼화 내부 담당자(Project.assignedManager)나 연구책임자(Project.researchLead)와는 별개의,
  // 그 기관 쪽에서 수수료·정산 관련 연락을 받는 사람이다.
  contactName: string;
  contactEmail: string;
  contactPhone: string;
  registeredAt: string;
  ...
  // 정산면제리스트(과기정통부 연구지원체계 등급 평가) 기준 참고 등급 — "정산면제리스트 업로드"로 갱신된다.
  // 과제별 실제 수수료 계산에 쓰이는 ProjectMember.institutionGrade(참여 시점 등급)와는 별개의 마스터 참고값이며,
  // 새 참여기관을 등록할 때 자동으로 채워주지 않는다 — 참여기관 등급은 여전히 과제별로 직접 입력/확인한다.
  referenceGrade?: "최우수(S)" | "우수(A)" | "우수(B)" | "우수(C)" | "일반";
}
...
export const fundingAgencies: FundingAgency[] = [
  ...
  },
  {
    ...
    specialNotes: [
      ...
      "일반수수료를 공동기관별 사업비 비율로 배분 — 공동기관별 개별 세금계산서 발행(분리 청구) 기능 구현됨",
...
export interface ProjectMember {
  ...
  // 등급 기본값 — 연차별 오버라이드(gradeOverrides)가 없는 연차에 쓰인다.
...
export const feePolicies: FeePolicy[] = [
  ...
  // ─── RDA2 (fa-006) 전담기관 자체 정책 ───────────────────────
  {
    ...
    description: "농촌진흥청 2유형(소속기관 트랙) — RDA1과 동일 기준(현금+현물, S등급 완전 제외). " +
      ...
      "공동기관별 개별 세금계산서 발행(분리 청구) 워크플로 구현됨 — 기관마다 세금계산서·공문발송·수금을 따로 관리.",
    createdAt: "2023-12-20",
    createdBy: "김관리",
    feeRateBrackets: RDA_BRACKETS,
    coInstAddonMethod: "TIERED" as const,
...
export interface TermFee {
  ...
  standardFee?: number; // 표준수수료 — 일반기관은 산정액과 동일, 면제기관은 85% 적용 전 원래 몫
...
export interface Receivable {
  ...
  termNumber: number;
  leadInstitutionId: string;
  leadInstitutionName: string; // 수금 대상 기관 (연차 통합이면 주관기관, 기관별 개별 관리면 그 기관)
  // RDA2처럼 기관별로 세금계산서·수금을 따로 관리하는 과제에서만 채워진다.
  // 없으면 기존처럼 연차 전체를 하나로 묶은 통합 레코드.
  institutionId?: string;
  billedAt: string;
  billedAmount: number;
  paidAmount: number;
...
export interface TaxInvoice {
  ...
  termNumber: number;
  leadInstitutionId: string;
  leadInstitutionName: string; // 세금계산서 수신자 (연차 통합이면 주관기관, 기관별 개별 관리면 그 기관)
  // RDA2처럼 기관별로 세금계산서를 따로 발행하는 과제에서만 채워진다.
  // 없으면 기존처럼 연차 전체(전 참여기관 합산)를 하나로 묶은 통합 레코드.
  institutionId?: string;
  issuedAt: string;
  supplyAmount: number; // 공급가액 (통합이면 참여기관 수수료 합산, 개별이면 그 기관 수수료)
