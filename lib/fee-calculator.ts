...
export function calcTermFee(input: CalcInput): CalcResult {
  ...
  // 2. 면제기관 분리 (DISCOUNT 모드 정책에서만) — 회계법인 실무 확인(2026-08-15)에 따라,
  // 면제/일반 분류는 등급이 아니라 **그 시점의 정산구분만으로** 정해진다: 자체정산이면 등급과
  // 무관하게 면제기관처럼(비례배분·청구비율 할인) 계산되고, 위탁정산이면 등급과 무관하게
  // 일반기관처럼(표준수수료 구간·배분) 계산된다. 등급(institutionGrade/gradeOverrides) 표시는
  // 이 계산 분류와 완전히 분리된 참고용 라벨이라 여기서는 전혀 참조하지 않는다 — 위탁 전환된
  // 기관의 등급 배지는 그대로 "우수" 등으로 남아있어도 계산은 일반기관 방식으로 진행되고,
  // 반대로 원래 "일반" 등급이라도 자체정산을 선택하면 면제기관처럼 계산된다.
  // EXCLUDE 모드 정책(IITP/RDA1/RDA2)은 면제등급이 이미 cashMembers 이전 단계에서 등급 기준으로
  // 완전히 제외되므로(isExcludedMember) 이 규칙과 무관하며, 여기서는 항상 빈 배열이 된다.
  const exemptMembers = policy.exemptionMode === "DISCOUNT" || policy.exemptionMode === "CUSTOM"
    ? cashMembers.filter((m) => m.settlementType === "자체정산")
    : [];
